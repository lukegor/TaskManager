...
{
    internal class ProcessManager : ObservableObject
    {
        ...
        private int _interval = MilisecondMultiplier *
            RefreshFrequencyTypeHelper.RefreshFrequencyTypeSecondsMapping[
                (RefreshFrequencyType)Convert.ToInt32(Properties.Settings.Default["RefreshRate"])];

        // Event handler to update the field when the setting changes.
        private void Default_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            if (e.PropertyName == nameof(Properties.Settings.Default.RefreshRate))
            {
                var mappingKey = (RefreshFrequencyType)Convert.ToInt32(Properties.Settings.Default.RefreshRate);
                _interval = MilisecondMultiplier * RefreshFrequencyTypeHelper.RefreshFrequencyTypeSecondsMapping[mappingKey];
        ...
        public ProcessManager()
        {
            ...
            _timer = new TimerManager(_interval);
        ...
        {
            var processList = await Task.Run(() => ProcessManager.GetProcesses());
        ...
        private static bool TryGetProcessBitness(IntPtr processHandle, out bool is32Bit)
        {
